var PAPERBOY_CONVERTER_VERSION = "0.1.2";
...
(() => {
  ...
  function tableToMarkdown(table2, service) {
    const matrix = [];
    for (let r = 0; r < table2.rows.length; r++) {
      const row = table2.rows[r];
      const cells = Array.from(row.cells).map((cell2) => cellToMarkdown(cell2, service));
      if (cells.length > 0)
        matrix.push(cells);
    }
    if (matrix.length === 0)
  ...
  function createTurndownService() {
    ...
    service.addRule("stripHeadingAnchor", {
      filter(node) {
        ...
        if (!parent || !/^H[1-6]$/.test(parent.nodeName))
          return false;
...
`;
  ...
  ParserBlock.prototype.tokenize = function(state2, startLine, endLine) {
    ...
    while (line < endLine) {
      ...
      let ok = false;
      for (let i = 0; i < len; i++) {
        ok = rules3[i](state2, line, endLine, false);
        if (ok) {
  ...
  function postProcess3(state2, delimiters) {
    const max = delimiters.length;
  ...
  function emphasis_post_process(state2) {
    const tokens_meta = state2.tokens_meta;
    const max = state2.tokens_meta.length;
    postProcess3(state2, state2.delimiters);
    for (let curr = 0; curr < max; curr++) {
      if (tokens_meta[curr] && tokens_meta[curr].delimiters) {
        postProcess3(state2, tokens_meta[curr].delimiters);
      }
    }
  }
...
`;
  ...
  function setStatus(text2, status = "idle") {
    dom.metaText.textContent = text2;
  ...
  function parseHtmlToDocument(html, url) {
    ...
    if (documentClone.head && url) {
      const base2 = documentClone.createElement("base");
      base2.href = url;
      documentClone.head.prepend(base2);
    }
